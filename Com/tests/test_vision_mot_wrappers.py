"""Wrapper import tests for mot compatibility modules."""
...
class VisionMotWrapperImportTest(unittest.TestCase):
    def test_root_mot_import(self):
        import mot

        self.assertTrue(hasattr(mot, "get_feature_vector"))
        self.assertTrue(hasattr(mot, "calc_cosine_similarity"))
        self.assertTrue(hasattr(mot, "ObjectTracker"))

    def test_wrapper_and_impl_are_same_objects(self):
        import mot
        from vision import mot as vision_mot

        self.assertIs(mot.get_feature_vector, vision_mot.get_feature_vector)
        self.assertIs(mot.calc_cosine_similarity, vision_mot.calc_cosine_similarity)
        self.assertIs(mot.ObjectTracker, vision_mot.ObjectTracker)
